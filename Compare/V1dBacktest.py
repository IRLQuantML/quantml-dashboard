...
def run_backtest(
    ...
    *,
    ...
    # Rank score once (used by salvage/top-K)
    try:
        df = add_ranking_columns(df, prediction_column, ticker=ticker)
    except Exception as e:
        logging.warning(f"{ticker} — ranking columns skipped: {e}")

    # 3) Apply signal filters (RSI/EMA/MACD/volume + prob/prob-diff if present)
    ...
    # funnel logs (unchanged)
    _log_filter_funnel(
        ticker=ticker,
        df=df,
        filters_info=filters_info,
        combined_mask=combined_mask,
        save_csv_dir=os.path.join(predict_path, "metrics", "filter_funnels")
    )

    # 4) Build execution rows (signals only)
    if filtered_df is None or filtered_df.empty:
        empty_trades = pd.DataFrame(columns=["entry_date","exit_date","entry_price","exit_price","pnl","return_pct","days_held","reason"])
        trades_filename = save_trade_log(empty_trades, ticker, model, suffix=f"{to_prefix(model)}")
        metrics = _empty_metrics()
        return float(initial_capital), empty_trades, metrics, trades_filename

    trades_df = build_trade_signals(
        filtered_df,
        signal_col=engine_pred_col,
        conf_col=prob_column if (prob_column and prob_column in filtered_df.columns) else None
    )

    # 5) Execute via the full‑timeline engine
    final_capital, realized_trades_df, metrics, trades_filename = backtest_engine(
        trades_df=trades_df,                       # entries (sparse)
        ticker=ticker,
        price_df=price_df_full,                    # daily timeline (dense)
        model_type=model,
        suffix=(suffix or to_prefix(model)),
        initial_capital=float(initial_capital),
        transaction_cost=float(transaction_cost),
        sl_mult=float(sl_mult),
        tp_mult=float(tp_mult),
        prediction_column="ml_signal",
        prob_column=(prob_column if prob_column in filtered_df.columns else None),
        risk_pct=float(risk_pct if risk_pct is not None else getattr(CFG, "RISK_PER_TRADE", RISK_PCT)),
        use_trailing_stop = bool(getattr(CFG, "BACKTEST_USE_TRAILING_STOP", True)),
        trail_pct=float(getattr(CFG, "TRAIL_ATR_MULT", 1.0)),
        use_ranking=bool(use_ranking),
        min_conf_floor=min_conf_floor,
    )
    return float(final_capital), realized_trades_df, metrics, trades_filename
